...
void Bpar(int xtopleftX, int xtopleftY, int utopleftX, int utopleftY, int vtopleftX, int vtopleftY, int size, int base) {
        ...
        if (size == base) {
                ...
                //printf("Bpar - %d,%d,%d,%d,%d,%d,%d,%d", xtopleftX, xtopleftY, utopleftX, utopleftY, vtopleftX, vtopleftY, size, base);
...
void Apar(int xtopleftX, int xtopleftY, int size, int base) {
	if (size > 1) {
                ...
                Apar(xtopleftX, xtopleftY, size / 2, base);
                cilk_spawn Apar(xtopleftX  + size / 2, xtopleftY  + size/2, size / 2, base);
                cilk_sync;
...
int main(int agrc, char* argv[]) {
	...
	cpu_time_used = ((double) (end - start)) / CLOCKS_PER_SEC;
        printf("CPU time used for matrix size = %d with base case = 1 and nworkers %d = is : %f\n", N, __cilkrts_get_nworkers(), cpu_time_used);
	//}
